...
export default function TrustBar({ settings }: TrustBarProps) {
  ...
  return (
    <section className="bg-ocean-900 border-t-4 border-red-500 py-6">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
          {items.map((item) => {
            const content = (
              <div className="flex items-center gap-3 justify-center text-center lg:text-left">
                <div className="text-sand-400 shrink-0">{item.icon}</div>
                <span className="text-sm sm:text-base font-heading font-semibold text-white">
                  {item.label}
                </span>
              </div>
            );
